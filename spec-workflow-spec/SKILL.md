---
name: spec-workflow-spec
description: "为新模块撰写活文档 spec.md 作为后续 change 的基线。通常由 /spec-workflow-module-init 触发。触发：写 spec / 需求分析 / 模块规格 / /spec-workflow-spec。NOT FOR: 修改已有模块（走 /spec-workflow-change，通过 Delta 演化 spec）。"
---

# spec-workflow-spec — 模块需求规格（活文档）
...
- **不用于已有模块的修改**：已有模块的变更走 `/spec-workflow-change`，通过 Delta 增量演化 spec
- 通常由 `/spec-workflow-module-init` 作为其一个步骤调用；独立调用时也只处理新模块
...
在撰写 spec 之前，检查已有知识避免重复踩坑：

1. 调用 `/spec-workflow-wiki check <module-topic>` 搜索全局 wiki 和本地项目经验
2. 重点关注：
   - `wiki/pitfalls/` 中与模块主题相关的已知问题
   - `.spec-workflow/lessons/` 中项目级经验教训
   - `wiki/patterns/` 中适用的设计模式
3. 将发现的相关经验整合到后续的需求澄清中
4. 如果发现高度相关的踩坑记录，在 spec.md 的"已知约束"或"开放问题"章节引用（使用 wikilinks 格式 `[[path]]`）
...
| 章节 | 标题 | 内容 |
|------|------|------|
| §1 | 概述 | 模块简述、所属系统、关联文档、**模块架构图（必画）** |
| §2 | 用户故事与场景 | 目标用户、用户故事、使用场景 |
| §3 | 功能需求 | 输入、输出、核心行为、业务规则、**核心流程图/状态图（条件必画）** |
| §4 | 非功能需求 | 性能、安全、兼容性 |
| §5 | 边界与限制 | In Scope、Out of Scope、已知约束 |
| §6 | 验收标准 | Given-When-Then 格式 |
| §7 | 多层协作需求 | 条件章节，仅 `spec-layers.md` 存在时生成 |
| §8 | 开放问题 | 未解决问题清单 |
| §9 | 关联变更 | 由归档流程自动维护 |
| §10 | 变更记录 | 变更历史表 |
...
- 图类型选择、命名、样式规范统一遵循项目内 mermaid 规范文档
- spec 阶段的图描述**当前稳定态**，不带 `:::new/:::changed` 标注（那是 change 的事）

## 关键约束

- **绝不写代码**：包括伪代码、代码片段、接口定义。这些属于 spec-workflow-change 阶段的职责
...
- **与 spec-workflow-change 的分工**：spec 只建一次基线；后续所有修改走 change 的 Delta
