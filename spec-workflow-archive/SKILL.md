---
name: spec-workflow-archive
description: "将已审查通过的 change 的 Spec Delta 合并回模块 spec.md，完成变更闭环。触发：归档 change / archive / 合并 delta / /spec-workflow-archive。"
---
...
- **必须能找到 `.eo-project.json`**（cwd 或父目录）。找不到 → 报错退出，提示运行 `/spec-workflow-project-init`。所有 `eo-doc/` 路径通过 `.eo-project.json` 的 `doc_root` 字段解析
- 用户给定 `<module-name>` 和 `<change-id>`（如 `/spec-workflow-archive inventory 015-add-sort`）
- `eo-doc/dev/<module-name>/spec.md` 存在
- `eo-doc/dev/<module-name>/changes/<change-id>/change.md` 存在且 `status: done`（companion change 放宽为 `approved`）
- 对应的 `review.md` 存在且审查通过（无 P0/P1 剩余）（companion change 豁免此要求）
- 对应的 `test.md` 存在且通过（若 change 要求了测试）（companion change 豁免此要求）
...
2. **从 frontmatter 读取 `change_type` 和 `companion`**，确定归档模式：
   - `bootstrap` → **bootstrap 模式**（跳过 Delta 合并，只更新元信息与索引）
   - `companion: true` → **companion 模式**（Delta 合并 + 豁免 implement/test/review）
   - `feature` / `enhance` / `refactor` → **Delta 合并模式**（执行后续所有步骤）
3. 读 `changes/<change-id>/review.md`，确认结论为通过（companion 模式跳过此步）
4. 读 `changes/<change-id>/test.md`（若存在），确认通过（companion 模式跳过此步）
5. 读模块 `spec.md`
...
**拒绝跨模块 Delta**：所有 Delta 条目的目标章节必须位于**本模块**的 `spec.md`。若出现以下任一信号，**停止归档**并要求用户拆 change：
...
反馈模板：
> 拒绝跨模块 Delta：本 change 的 Delta 含跨模块条目（`<条目名>` 指向 `<其他模块>/spec.md`）。
> spec-workflow-archive 只能合并单一模块的 Delta。请按 `/spec-workflow-change` 的"单模块 vs 跨模块"规则：
>   1. 将跨模块部分抽出为另一个 change（归属对方模块）
>   2. 用 `depends_on` frontmatter 关联两个 change
>   3. 按依赖顺序分别归档
> 详见 `spec-workflow-change/SKILL.md` 的"判断边界：单模块 vs 跨模块"。
...
**ADDED 项**：确认目标章节存在；若目标章节不存在，提示用户先修 change.md 或允许追加到章节末尾。**companion 豁免**：若 Delta 目标为"被依赖声明"章节且 spec.md 中不存在，自动在末尾（关联变更之前）创建该章节（含表头）。
...
- **ADDED**：在指定章节末尾追加新条目。companion 模式下若"被依赖声明"章节不存在，先创建章节（含表头），再追加条目
...
3. 在 `## 10 变更记录` 追加一行：
   - Delta 模式：`| YYYY-MM-DD | 归档 <change-id>: <一句话描述> | spec-workflow-archive |`
   - bootstrap 模式：`| YYYY-MM-DD | bootstrap 实现 <认领章节列表> (<change-id>) | spec-workflow-archive |`

### 第六步：更新 change.md

1. frontmatter `status` 改为 `archived`（普通 change 从 `done` 改；companion change 从 `approved` 改）
2. 若非 companion change，`## 10 实施记录` 填入归档日期
...
**companion 模式**：
- 合并的 Delta（通常为 1 条 ADDED，追加到"被依赖声明"章节）
- 消费方模块与依赖能力
- change 状态已改为 archived
...
| 触发条件 | 建议文案 |
|---------|---------|
| MODIFIED >= 3 条 或 REMOVED >= 1 条 | 本次 Delta 对 spec 做了 N 条 MODIFIED / M 条 REMOVED，建议跑一次 `/spec-workflow-spec-review <module-name>` 验证新基线仍然自洽 |
| 涉及 spec 章节 >= 3 个 | 本次 Delta 触及 spec 的 N 个章节，建议跑一次 `/spec-workflow-spec-review` 确认跨章节一致性 |
| 仅少量 ADDED | 无需复检建议 |
