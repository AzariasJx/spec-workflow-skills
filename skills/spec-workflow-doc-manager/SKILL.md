---
name: spec-workflow-doc-manager
description: "管理 eo-doc/ 代码侧文档体系（init / sync / re-sync / modify / query）。所有 eo-doc 下的文档操作必须走此 skill。触发：初始化文档 / 同步文档 / 查文档 / /spec-workflow-doc-manager。"
---

# spec-workflow-doc-manager

**代码侧**文档管理。项目管理侧（roadmap / decisions / lessons / design / docs）由 `spec-workflow-project-*` skill 管。

## 前置

除 `init` 外的所有命令必须能找到 `.eo-project.json`（cwd 或父目录）。找不到 → 报错退出，提示运行 `/spec-workflow-project-init`。

`init` 通常由 `/spec-workflow-project-init` 内部调用；用户直接调用 `/spec-workflow-doc-manager init` 时，若 `.eo-project.json` 不存在会提示先走 `/spec-workflow-project-init`。
...
**路由规则**：
1. 明确命令（如 `/spec-workflow-doc-manager sync`） → 直接路由
2. 自然语言 → 按触发词匹配
3. 无法判断 → 列出可用命令
...
│   └── INDEX.md
├── dev/              # 必建，spec/change/review 流（由 spec-workflow-* 工作流 skill 维护）
│   └── INDEX.md
├── templates/        # 必建（空），spec-workflow-* 技能扩展点
...
| `templates/` | spec-workflow-* 技能的扩展点 — 项目类型、层级结构、工作流定制 | AI | "项目**怎么**定制？" | — |
...
**templates/**：
- 不是文档，是 spec-workflow-* 技能的扩展点
- 定义项目类型（`project-profile.md`）、多层对齐模板（`spec-layers.md`）、分层 Part 模板（`plan-layers.md`）、分层执行模板（`implement-layers.md`）
- 模板可选，不存在时 spec-workflow-* 技能使用内置默认行为
- sync / re-sync 不处理 templates/（它们不是从源码生成的）

**dev/**：
- 子目录由 spec-workflow-module-init、spec-workflow-change、spec-workflow-implement、spec-workflow-review 等技能按约定产出
...
通常由 `/spec-workflow-project-init` 内部调用。直接调用时：

1. 检查 `.eo-project.json` 是否存在；不存在 → 提示先走 `/spec-workflow-project-init` 并退出
...
8. **不自动生成 state/ 和 agent-handbook/ 内容**——留待 `/spec-workflow-doc-manager sync` 或 `re-sync` 首次触发
