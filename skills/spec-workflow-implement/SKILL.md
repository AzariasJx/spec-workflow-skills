---
name: spec-workflow-implement
description: "根据 change.md 的 TODO 落地代码；也负责 change 生命周期内所有 bug 修复（fix 是 implement 的职责，不开新 change）。触发：实现 / 写代码 / implement / fix / 修 bug / /spec-workflow-implement。"
---
...
- 若 change 不存在 → 提示用户先执行 `/spec-workflow-change`
...
1.5 **查询相关经验**
   在动手写代码之前，检查已有知识避免重复踩坑：
   1. 调用 `/spec-workflow-wiki check <implementation-topic>` 搜索全局 wiki 和本地项目经验
   2. 重点关注：
      - `wiki/pitfalls/` 中与实现技术相关的已知坑（如特定库的版本问题、平台兼容性等）
      - `.spec-workflow/lessons/` 中项目级经验（特别是同模块历史偏差记录）
      - 该模块已有 change 的 `implement.md`（历史偏差）
   3. 如果发现高度相关的踩坑记录，在实现过程中主动规避，必要时记录到当前 change 的 implement.md
   4. 不因为经验查询结果擅自修改 change 的 §3 或 TODO —— 如有冲突上报用户
...
7. **完成标记**
   - 所有 TODO 勾选完成后，告知用户进入 `/spec-workflow-test`
...
3. 每修复一个问题，运行对应验证
4. 修复完成后通知用户重新 `/spec-workflow-test` 或 `/spec-workflow-review`
