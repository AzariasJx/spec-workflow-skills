---
name: spec-workflow-backlog
description: "往项目 backlog.md 追加待办、灵感或未接入的未来规划。通过 .eo-project.json 定位项目。触发：加入 backlog / 记一条待办 / 以后再说 / /spec-workflow-backlog。"
---
...
**只做"追加"**。决策记录走 `spec-workflow-project-update`，经验教训走 `spec-workflow-project-lesson`，本 skill 不越界。

配置与目录约定见 [spec-workflow-project-init/SKILL.md](../spec-workflow-project-init/SKILL.md)。

## 前置

**必须**能找到 `.eo-project.json`（cwd 或父目录）。找不到时报错退出，提示运行 `/spec-workflow-project-init`。

`backlog.md` 由 `spec-workflow-project-init` 初始化时必建，理论上一定存在。若丢失则按以下模板重建一份空骨架再写入：
...
- **不建 `decisions/` / `lessons/`**（那是 `spec-workflow-project-update` / `spec-workflow-project-lesson` 的职责）
