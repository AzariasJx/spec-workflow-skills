...
description: "对已实施的代码做审查，产出 P0/P1/P2 分级报告（前提：代码已实现）。触发：review / 代码审查 / /spec-workflow-review。NOT FOR: spec 审查（/spec-workflow-spec-review）、change 方案审查（/spec-workflow-change-review，代码还没写时用）。"
---
...
> **定位**：`spec-workflow-review` 只审代码。审查范围前移（审 spec / 审 change 方案）请用 `/spec-workflow-spec-review` / `/spec-workflow-change-review`。三种 review 关注点、上下文、产出物都不同，不要混用。
>
> | Skill | 审查对象 | 产出 |
> |-------|---------|------|
> | `/spec-workflow-spec-review` | 模块 spec.md | `spec-review.md` |
> | `/spec-workflow-change-review` | change.md（implement 前） | `change-review.md` |
> | **`/spec-workflow-review`**（本技能） | change 实施后的代码 | `review.md` |
...
- **相关代码已实现**（即 `/spec-workflow-implement` 已经跑过至少一轮，change.md 的 TODO 至少部分被勾选）
...
| 信号 | 含义 | 正确路径 |
|------|------|---------|
| change.md `status: draft` | change 还没 approve，更没 implement | 走 `/spec-workflow-change-review` |
| change.md `status: approved` 但所有 TODO 未勾选 | 代码还没开工 | 走 `/spec-workflow-implement` 先实施 |
| 用户描述是"审查 change 方案" / "审方案" / "implement 之前再看看" | 用户想要的是方案审查 | 走 `/spec-workflow-change-review`（**不是本 skill**） |
| 用户描述是"change 重写后再看看有没有新问题" | 代码未变，只是 change.md 改了 | 走 `/spec-workflow-change-review` |

纠偏反馈模板：
> `/spec-workflow-review` 是**实施后的代码审查**，需要代码已经写出来。
> 你当前的情况是 `<信号描述>`——这一步应该走 `/spec-workflow-change-review`（方案审查）。
> 两者区别：
> - `/spec-workflow-change-review` 审 `change.md` 本身（Delta / TODO / AC 是否合规） — **implement 之前**
> - `/spec-workflow-review`（本 skill）审**代码** — implement / test 之后
...
按照固定模板撰写，写入 `eo-doc/dev/<module-name>/changes/<change-id>/review.md`。

模板见 [references/review-template.md](references/review-template.md)。

## 关键约束

- **客观公正**：基于文档和最佳实践审查，不做主观偏好评判
- **问题定位精确**：必须给出具体的文件路径和行号
- **不直接改代码**：Review 只产出报告，修复工作由 `/spec-workflow-implement` 执行
