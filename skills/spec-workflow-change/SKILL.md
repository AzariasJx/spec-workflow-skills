...
- 如果模块不存在 → 提示用户先执行 `/spec-workflow-module-init <module-name>` 完成模块初始化
- 如果模块 spec 存在但 `status: draft` → 提示用户先完成 spec-review
- 如果用户传了 `--from <proposal-id>`，需要 requirement-brief.md 存在且 status 为 `elicited` 或 `eliciting`（`consumed` 状态的 brief 不允许再次消费，除非用户明确要求覆盖）

## 工作流程

### 前置步骤：检查 --from 参数（可选）

若用户执行 `/spec-workflow-change <module> --from <proposal-id>`：

1. 定位 requirement-brief.md：
   - 先查 `eo-doc/dev/<module>/proposals/<proposal-id>/requirement-brief.md`
...
4. 若无匹配：提示用户走 `/spec-workflow-module-init` 创建新模块，暂停本流程
5. **关键决策：跨模块必拆**（详见下方"判断边界：单模块 vs 跨模块"）
   - 只调用别的模块、不改对方 spec → **主 change + 每个被依赖模块一个 companion change**
   - 多个模块 spec 都需要 Delta → **拆多个 change**，每模块一个，用 `depends_on` 关联
6. 若判定为跨模块拆分 → 向用户明确说明"会产出 N 个 change"（含 companion），并按 `depends_on` 顺序依次产出
...
3. 识别变更类型，按以下矩阵判断：
   ...
   - **不接受 `fix` 作为类型**：若用户描述是"修 bug"——属于对某个已归档 change 的实施缺陷修复，不是新 change：
     - 若相关 change 尚未归档 → 让用户回到 `/spec-workflow-implement` 继续修
     - 若已归档但确实是"实现不符合 spec" → 仍走 spec-workflow-implement 开补丁式实施（不建新 change）
     - 若归档后发现的是 spec 本身错了（业务语义变化）→ 那就是真正的 `enhance`，按 enhance 处理
4. **执行模板发现**（见上方）

### 第二步点五：查询相关经验

在撰写 change 之前，检查已有知识避免重复踩坑：

1. 调用 `/spec-workflow-wiki check <change-topic>` 搜索全局 wiki 和本地项目经验
2. 重点关注：
   - `wiki/pitfalls/` 中与变更主题相关的已知问题
   - `.spec-workflow/lessons/` 中项目级经验教训
   - 该模块 `changes/` 下历史 change 的 implement.md（偏差记录）
3. 将发现的相关经验融入后续的澄清环节
4. 如果变更涉及已有踩坑记录中提到的问题，在 change.md 的"影响评估"或"风险与缓解"章节引用（使用 wikilinks 格式）
...
2. 不需要立刻回写 module 的 `spec.md`——合并动作由 `spec-workflow-archive` 在归档时执行
...
- **高风险 / 大规模 / 多层协作 change**（Delta >= 5 条，或跨 2 个以上层，或含 MODIFIED/REMOVED，或 change_type = bootstrap 认领章节 >= 3 个）：**建议**用户先跑 `/spec-workflow-change-review`
- **小型 / 低风险 change**（Delta <= 2 条，单层，全是 ADDED）：可直接 approved 进入 implement

统一提示模板：
> change 文档已就绪（`status: draft`）。后续流程：
>
> **（可选，建议）** `/spec-workflow-change-review <change-path>` — 方案级审查（§3 合规、TODO 完整、AC 覆盖），通过后再 approve
> 1. 将 change.md `status` 改为 `approved`
> 2. `/spec-workflow-implement <change-path>` — 按 TODO 实施代码
> 3. `/spec-workflow-test <change-path>` — 测试与验证
> 4. `/spec-workflow-review <change-path>` — **实施后代码审查**（仅当 implement 已跑过）
> 5. `/spec-workflow-archive <module-name> <change-id>` — 代码审查通过后归档

#### 场景 B — 返工修订（作者根据 change-review 的 P0/P1 回来改 change.md）

**关键：此时代码尚未实施，必须走 `/spec-workflow-change-review` 复审，不要走 `/spec-workflow-review`。**

`/spec-workflow-review` 是实施后的代码审查，需要代码已经写出来；此时根本没代码，路径走错会让 reviewer 空审。

统一提示模板（场景 B）：
> change 重写完成（`status: draft`）。既然前一轮 change-review 发现了问题，修订后请**再跑一次** `/spec-workflow-change-review <change-path>` 确认问题已收敛。
>
> `/spec-workflow-change-review <change-path>` — 复审方案（**不是 /spec-workflow-review**；代码还没写，没东西给 /spec-workflow-review 审）
> 直到 change-review 无 P0/P1 → 用户改 `status: approved` → 进入 `/spec-workflow-implement`
>
> **禁止**此时直接改 `status: approved` 跳过复审，也**禁止**跳到 `/spec-workflow-implement` 或 `/spec-workflow-review`。
...
| 信号 | 走 spec-workflow-change | 走 spec-workflow-module-init |
|------|-------------------------|------------------------------|
| 目标模块已存在 spec | 是 | 否 |
| 目标模块完全不存在 | 否（先 init） | 是 |
| 是对已有能力的修改 | 是 | 否 |
| 是全新模块的首次落地 | 否 | 是 |

当模块 spec 需要大规模结构性重写（Delta 占 spec 80% 以上）时，不要跳回 `spec-workflow-module-init`，而是用 `change_type: refactor` 发一个 change，逐步演化。
...
**核心准则：跨模块必拆。**
一个 change 结构上只能归属一个模块，Delta 只能合并到一份 `spec.md`。只要涉及其他模块（无论是否改对方 spec），都必须在对方模块创建 companion change 建立反向追踪。

| 场景 | 归属 | 做法 |
|------|------|------|
| **A. 只调用不改契约** — 新 change 调用对方模块已有的接口/能力 | **主 change + companion** | 主 change 归消费方模块；每个被依赖模块各创建一个 companion change，记录被依赖声明 |
| **B. 多模块 spec 都要动** — 需要在多个模块的 spec 同时 ADDED/MODIFIED/REMOVED | **拆多个 change**，每模块一个 | 用 `depends_on` frontmatter 和共享命名关联 |
| **C. 只动 Proto/Config，不改业务模块 spec** | 归调用方模块 | Proto/Config 不是一等模块，归属消费侧 |

### Companion Change（伴随变更）

当主 change 调用了其他模块的能力（Scenario A），需要为每个被依赖模块创建一个 lightweight companion change，在对方 spec 的"被依赖声明"章节建立反向追踪。

**目的**：下次被依赖模块迭代时，spec 里已有"谁在用我"的上下文，避免无感知改坏下游。

#### companion change 要素

1. **归属**：放在被依赖模块的 `changes/` 下
2. **change-id**：`<NNN>-companion-<主change语义名>`（如 `003-companion-smart-ticketing`）
3. **frontmatter**：
   ```yaml
   companion: true
   companion_for: <消费方模块>/<主change-id>
   related:
     - <消费方模块>/<主change-id>
   ```
4. **§3 Delta**：单条 ADDED，目标为 spec 的"被依赖声明"章节：
   ```
   ADDED 被依赖声明
   - 消费方：<消费方模块>
   - 依赖能力：<具体 API / 接口描述>
   - 来源 change：<主change-id>
   ```
5. **无 TODO**：companion change 不涉及代码实施，§4 写"本 change 不涉及代码实施，仅记录被依赖关系"
6. **流程快车道**：draft → approved → 直接 archive（跳过 implement / test / review）

#### companion change 创建时机

在主 change 的第五步（撰写 change.md）完成后，**紧接着为每个 §2.3 "依赖的外部模块"表中列出的模块创建 companion change**。两者在同一轮 `/spec-workflow-change` 调用中完成。

#### 第八步提示补充

产出 companion change 后，在主 change 的提示后追加：
> 已为以下被依赖模块创建 companion change：
> - `<模块>/<change-id>` — 依赖 <能力描述>
>
> companion change 无需 implement/test/review，直接 `/spec-workflow-archive <module-name> <companion-change-id>` 即可归档。
...
4. **实施时序**：被依赖方先 approve → implement → archive → 再 approve 依赖方。`/spec-workflow-workflow implement` 一次只跑一个 change，天然强制顺序

### 反模式

- **建 `dev/cross/` 虚拟模块**把跨模块需求塞进去 — 破坏"模块一等公民"，archive 无法定位目标 spec
- **建 umbrella change 引用多个子 change** — 多一层没带来收益，Delta 合并该拆还是得拆
- **单个 change 挂多个模块的 Delta** — `/spec-workflow-archive` 会拒绝（它只认单一目标 spec）
- **Scenario A 不建 companion** — 下次被依赖模块迭代时失去"谁在用我"的上下文，可能改坏下游
...
- 规范统一见 [spec-workflow-doc-manager/references/mermaid.md](../spec-workflow-doc-manager/references/mermaid.md)
- 归档时由 spec-workflow-archive 清除 `:::new/:::changed` 后并入 spec.md §3.5
...
- **状态流转**：draft → approved（用户确认）→ implementing（spec-workflow-implement 启动时改）→ done（审查通过）→ archived（spec-workflow-archive 完成）
- **不改模块 spec**：change 阶段不直接修改 `spec.md`，合并由 `spec-workflow-archive` 负责
