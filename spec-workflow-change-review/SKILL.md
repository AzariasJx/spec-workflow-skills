---
name: spec-workflow-change-review
description: "对 change.md 做方案级审查（Delta 正确性、TODO 完整性、AC 覆盖）..."
---

# spec-workflow-change-review — Change 方案审查
...
| Skill | 审查对象 | 问的问题 |
|-------|---------|---------|
| `spec-workflow-spec-review` | 模块 `spec.md`（活文档基线） | 需求对不对？业务自洽吗？ |
| **`spec-workflow-change-review`**（本技能） | 某个 change 的 `change.md` | 方案对不对？Delta 和实施方案一致吗？ |
| `spec-workflow-review` | change 实施后的代码 | 代码对不对？实现 vs AC？ |

三者关注点、上下文、回退动作完全不同，**不要混用**。

## 核心原则

1. **方向级审查，不替作者做决定**：审查只产出报告，方案修订由用户决定后重跑 `/spec-workflow-change`
...
> **不要主观追问审查模式**：`change_type` 是确定性字段，必须从 change.md 直接读取并据此分派。**禁止**向用户提问"这份 change 是在改 spec 还是只是拆实施批次"——若 change.md 没填 `change_type` 或填错，应作为 P0 问题报告（"frontmatter `change_type` 缺失/不匹配"），而不是反向追问。
...
- **反模式检测**：是不是把整段 spec 抄了一遍当成 ADDED？Delta 应该是"增量"，不是"新 spec"
  - 若 Delta 大段复述 spec 现有内容 → 也许根本不该是 Delta 模式，建议作者改为 `bootstrap` 类型
- **跨模块拒绝**：所有 Delta 条目的 spec 路径必须指向**本模块**；指向其他模块 → P0（详见 spec-workflow-change "判断边界：单模块 vs 跨模块"）
...
向用户汇报，**严格按下面两种终态二选一措辞**（避免含糊指向 `/spec-workflow-review`）：

**终态 1 — 通过（无 P0/P1）**：
> change-review 通过（P0=0, P1=0, P2=N）。
> 下一步：用户将 `change.md` 的 `status` 从 `draft` 改为 `approved`，然后跑 `/spec-workflow-implement <change-path>`。
> （注意：`/spec-workflow-review` 是**代码**审查，要在 `/spec-workflow-implement` 之后跑，现在还不轮到它。）

**终态 2 — 需修订（有 P0/P1）**：
> change-review 发现 P0=N / P1=M 个问题，不能进入 implement。
> 下一步：用户回到 `/spec-workflow-change <change-path>` 按本报告 P0/P1 逐条修订，修订完成后**再跑一次** `/spec-workflow-change-review <change-path>` 复审。
> **不要**直接改 `status: approved`，**不要**跑 `/spec-workflow-review`（代码还没写），**不要**跑 `/spec-workflow-implement` 跳过复审。
> 循环直到本报告 P0=0 且 P1=0。

**严禁**在终态 2 的措辞里出现"下一步跑 /spec-workflow-review" 或"可进入 implement"——这是高频踩坑点，agent 会把 `/spec-workflow-review`（代码审查）和 `/spec-workflow-change-review`（方案审查）串线，导致用户走错路径。
...
明确结论：可进入 implement / 需小幅修订后进入 / 需大幅修订
...
|-----------|------|-----------|------|------|
| <能力名1> | ADDED | §X.Y | pass / needs revision | ... |
| <能力名2> | MODIFIED | §X.Y | pass / needs revision | 旧描述在 spec 中是否存在 |
| <能力名3> | REMOVED | §X.Y | pass / needs revision | ... |

**实现范围表（bootstrap）**

| 认领章节 | 认领范围 | 章节是否存在 | 是否被其他 bootstrap 认领 |
|---------|---------|------------|-------------------------|
| spec §3.1 | 完整 / 子集 | pass / fail | 无冲突 / 与 `NNN-xxx` 重叠 |
...
| §3 条目 | 对应 TODO | 覆盖状态 |
|--------|-----------|---------|
| ADDED-1 / §3.1 | TODO-S1, TODO-C2 | 完整覆盖 / 部分覆盖 / 遗漏 |

## AC 覆盖检查

| Delta 条目 | 对应 AC | 覆盖状态 |
|-----------|---------|---------|
| ADDED-1 | AC-1, AC-2 | 已覆盖 / 缺失 / 不完整 |

## 结构完整性检查

| 章节 | 状态 | 备注 |
|------|------|------|
| §1 变更意图 | pass / fail / needs revision | ... |
| §2 范围（In/Out Scope） | pass / fail / needs revision | ... |
| §3 Spec Delta | pass / fail / needs revision | ... |
| §4 实施方案 / TODO | pass / fail / needs revision | ... |
| §5 验收标准 | pass / fail / needs revision | ... |
| §6 测试标准 | pass / fail / needs revision | ... |
| §7 影响评估 | pass / fail / needs revision | ... |
| §8 风险与缓解 | pass / fail / needs revision | ... |
```
...
- **不改 change.md**：审查只产出报告，修订由用户回到 `/spec-workflow-change` 执行
